"""Final LIBERO-10 Table 2 runner.

Runs the learned continuous observable trigger or the oracle-onset upper bound
with identical corrected disturbance handling, synchronous and asynchronous
baselines, the deployed state/visual handoff, and configurable OFT takeover k.
Every output records the driver and artifact hashes used for the run.
"""
import argparse
import hashlib
import json
...
from asyncmixvla.runtime_io import MAX_STEPS, call_policy, get_env_and_task, prepare_observation, process_action, warm_up
from asyncmixvla.runtime_support import EpisodeEnv
...
TRIGGER_MARK = {"v1_v2_cascade_continuous": "v1v2cont_corrected", "oracle_onset": "oracle_corrected"}
...
RUNTIME_TRIGGER = {"v1_v2_cascade_continuous": "observable_cascade_continuous", "oracle_onset": "oracle_onset"}
...
def run_corrected_episode(point, method, trigger_name, checkpoint, max_steps=MAX_STEPS,
    ...
    """Run one matched benchmark cell with the selected trigger and method."""
    ...
    for pi, point in enumerate(points):
        for m in methods:
            ...
            rec = {"tag": tag, "method": m, "trigger": args.trigger, "bench_point": point,
                   "runtime": "asyncmixvla.runtime_support.EpisodeEnv",
