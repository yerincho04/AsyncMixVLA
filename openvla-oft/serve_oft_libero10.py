...
async def act_with_action_consistency(request: Request):
    """Apply the causal action-consistency residual to OFT's current visual latent."""
    ...
    with torch.inference_mode():
        ...
        z_now_full_np = z_now_full.float().cpu().numpy()[0]
...
async def act(request: Request):
    ...
    # Match the evaluated OFT preprocessing:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the frozen action-consistency residual checkpoint (convert_C_residual_to_checkpoint.py). "
             ...
             "including /act, is unaffected either way.",
    )
