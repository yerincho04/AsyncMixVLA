...
def make_dataset_from_rlds(
    ...
    *,
    ...
    has_native_val_split = "val" in available_splits
    if has_native_val_split:
        split = "train" if train else "val"
    else:
        # Some RLDS conversions (e.g. openvla/modified_libero_rlds) publish
        # only a "train" split with no held-out "val" split. Fall back to a
        # percentage-based sub-split of "train" for episode-level validation
        # instead of crashing on an unknown split name -- TFDS's split
        # slicing partitions by underlying shard/example boundaries, so this
        # remains a genuine non-overlapping trajectory-level split. Both the
        # train and val sides are sliced here (90/10) so they stay disjoint;
        # datasets that do publish a real "val" split are unaffected (branch
        # above uses the full, unsliced "train" split as before).
        split = "train[:90%]" if train else "train[90%:]"
