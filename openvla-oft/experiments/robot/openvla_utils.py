...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    # Idempotency check (added for safe concurrent job execution against a
    # shared checkpoint directory, e.g. multiple LIBERO-Long task jobs all
    # loading the same OFT/Adapter checkpoint at once): if a prior call
    # (this process or another concurrent one) already wrote the correct
    # auto_map, skip the backup+rewrite entirely. Without this, every
    # concurrent get_vla()/get_model() call unconditionally re-backs-up and
    # rewrites config.json, racing on the same file. This makes steady-state
    # concurrent loads pure reads (no write), while a genuinely fresh
    # checkpoint still gets backed up and updated exactly as before.
    ...
    # config.json -- os.replace is atomic on POSIX filesystems.
